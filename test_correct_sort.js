// 测试正确的排序逻辑（点赞数相同时，新帖子排在前面）

function testCorrectSort() {
    console.log('开始测试正确的排序逻辑...');
    ...
    // 正确的排序（点赞数降序，时间降序）
    console.log('\n=== 正确排序（点赞数降序，时间降序） ===');
    const correctSort = [...mockPosts].sort((a, b) => {
        // 首先按点赞数降序排序
        if (b.likeCount !== a.likeCount) {
            return b.likeCount - a.likeCount;
        }
        // 点赞数相同时，按时间降序排序（新帖子在前）
        return new Date(b.time) - new Date(a.time);
    });
    
    correctSort.forEach((post, index) => {
    ...
    console.log('\n=== 验证排序规则 ===');
    let isValid = true;
    for (let i = 1; i < correctSort.length; i++) {
        const prevPost = correctSort[i - 1];
        const currentPost = correctSort[i];
        ...
        if (prevPost.likeCount < currentPost.likeCount) {
            ...
            if (prevTime < currentTime) {
                ...
                console.log(`✅ 点赞数相同(${currentPost.likeCount})，时间排序正确（新帖子在前）`);
            }
        } else {
    ...
    // 显示排序逻辑
    console.log('\n=== 排序逻辑说明 ===');
    console.log('1. 首先按点赞数降序排序（点赞数多的排在前面）');
    console.log('2. 点赞数相同时，按发布时间降序排序（新帖子排在前面）');
    ...
    console.log('- 正确排序：帖子5（新的）排在帖子4前面');
